---
title: "DESI Ovarian Preprocessing"
...
ovarian_sample_dir <- gsub("\\\\", "/", r"(C:\Users\Jacob\Documents\DESI Ovarian\files)")
...
classes <- gsub(file.path(ovarian_sample_dir, "/"), "", list.dirs(ovarian_sample_dir, recursive=FALSE), fixed=TRUE)
...
file_name_list <- lapply(classes, function(class_dir) {
  files <- list.files(path = file.path(ovarian_sample_dir, class_dir), 
                      pattern = "\\.csv$|\\.xlsx?$", 
                      full.names = TRUE)
  sort(files)
})
