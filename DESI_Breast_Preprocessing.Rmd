---
title: "DESI Breast Preprocessing"
...
breast_sample_dir <- gsub("\\\\", "/", r"(C:\Users\Jacob\Box\Eberlin_Lab_BCM\People\Keziah\GGR project\Breast\DESI-MSI\Multi center extracted data)")
...
classes <- gsub(file.path(breast_sample_dir, "/"), "", list.dirs(breast_sample_dir, recursive=FALSE), fixed=TRUE)
```

```{r file and sample names}
## List file names
file_name_list <- lapply(classes, function(class_dir) {
  files <- list.files(path = file.path(breast_sample_dir, class_dir), 
                      pattern = "\\.csv$|\\.xlsx?$", 
                      full.names = TRUE)
  sort(files)
})
