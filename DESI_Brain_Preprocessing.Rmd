---
title: "DESI Brain Preprocessing"
...
brain_sample_dir <- gsub("\\\\", "/", r"(C:\Users\Jacob\Box\Eberlin_Lab_BCM\Projects\Brain\Extracted data\Extracted DESI)")
...
classes <- gsub(file.path(brain_sample_dir, "/"), "", list.dirs(brain_sample_dir, recursive=FALSE), fixed=TRUE)
```

```{r file and sample names}
## List file names
file_name_list <- lapply(classes, function(class_dir) {
  sample_dirs <- list.dirs(file.path(brain_sample_dir, class_dir), recursive = FALSE, full.names = TRUE)
  files <- unlist(lapply(sample_dirs, function(sample_dir) {
    list.files(sample_dir, pattern = "\\.csv$|\\.xlsx?$", full.names = TRUE)
  }))
  sort(files)
})
